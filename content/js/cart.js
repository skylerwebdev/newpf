...
var cart = (function() {
			...
			handler: function(args) {
				...
				if ((res.payload && res.payload.items) || (res.cart_details && res.cart_details.items) ) {
					...
					document.dispatchEvent(addToCartSuccessEvent);
				        ...
				        // temp fix for cart not clear when payment was completed
					if((res.error && res.error.code === 253916) || (res.cart_details && res.cart_details.code ===  253916)){
						document.cookie="zcid=;expires=Thu, 01-Jan-70 00:00:01 GMT;domain="+document.domain+";path=/;";
						_addProductToCart.call(addToCartButton);
					}
					else{
				...
				if (res.status_code === "0") {
					...
					for (var i in items) {
						...
						if(params.product_variant_id === item.variant_id) {
							...
							subtotal.innerHTML = item.approximate_total_formatted; // No I18N
	...
	var _validateSearch = function(e) {
        ...
        var searchInput = ($D.get('[data-zs-search-input]')) ? $D.get('[data-zs-search-input]') : this.form && this.form[0]; // No I18N
        var searchButton = ($D.get('[data-zs-search]')) ? $D.get("[data-zs-search]") : this.form && this.form[1]; // No I18N
		...
		if(window.zs_view == "checkout" || window.zs_view == "cart") {
			...
			for(var i = 0; i < line_items.length; i++) {
				...
				if(line_item_options && line_item_options.length > 0) {
					...
					item.id = (url_param_split) ? url_param_split[0] : "";
